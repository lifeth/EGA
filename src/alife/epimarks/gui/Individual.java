...
public class Individual extends JPanel {
	...
	public void paint(Graphics g) {
		...
		//Energy Units
		for (int i = 0; i < this.structure.metabolicCell.getMolecules().get(0).size(); i++) {
			Molecule mo = this.structure.metabolicCell.getMolecules().get(0).get(i);
		...
		//Receptors
		for (int i = 0; i < this.structure.transportCell.getMolecules().get(0).size(); i++) {
			Molecule mo = this.structure.transportCell.getMolecules().get(0).get(i);
			g2.drawImage(mo.getImage(), mo.getPosition().x,  mo.getPosition().y, mo.getDimension().width,  
			mo.getDimension().height, null);
		}
		
		//Transporters
			for (int i = 0; i < this.structure.transportCell.getMolecules().get(1).size(); i++) {
				Molecule mo = this.structure.transportCell.getMolecules().get(1).get(i);
				g2.drawImage(mo.getImage(), mo.getPosition().x,  mo.getPosition().y, mo.getDimension().width,  
				mo.getDimension().height, null);
			}
		...
		//Receptors
		for (int i = 0; i < this.structure.consumptionCell.getMolecules().get(0).size(); i++) {
			Molecule mo = this.structure.consumptionCell.getMolecules().get(0).get(i);
			g2.drawImage(mo.getImage(), mo.getPosition().x,  mo.getPosition().y, mo.getDimension().width,  
			mo.getDimension().height, null);
		}
		...
		//Receptors
		for (int i = 0; i < this.structure.reproductiveCell.getMolecules().get(0).size(); i++) {
			Molecule mo = this.structure.reproductiveCell.getMolecules().get(0).get(i);
			g2.drawImage(mo.getImage(), mo.getPosition().x,  mo.getPosition().y, mo.getDimension().width, 
			mo.getDimension().height, null);
		}
		...
		g2.fill3DRect(newpoint.x, newpoint.y, square.width, square.height, true);
		//g2.draw(new Rectangle2D.Float());

		g2.setPaint(Color.BLACK);
		g2.setRenderingHint(RenderingHints.KEY_ANTIALIASING, RenderingHints.VALUE_ANTIALIAS_ON);
		g2.fillOval(bpoint.x, bpoint.y, ball.width, ball.height);*/
	}
	...
	public class Structure {
		 ...
		 * @throws Exception 
		 */
		public Structure(Cell metabolicCell, Cell transportCell, Cell consumptionCell, Cell reproductiveCell) throws Exception {
